...
def set_season_years(season_start_year, season_end_year):
    ...
    print(season_start_date_int)
    print(season_end_date_int)
...
#train = matches[(matches["gameDate"] >= 20081004) & (matches["gameDate"] < 20231010)]
...
acc = accuracy_score(test["winner"], predictions)
print("Accuracy of predicting the outcome with " , predictors, " as the predictors: ", round(acc, 4))
...
def make_predictions(data, predictors):
    ...
    train = data[(data["gameDate"] >= 20081004) & (data["gameDate"] < 20231010)]
    test = data[(data["gameDate"] >= 20231010) & (data["gameDate"] < 20240625)]
    rf.fit(train[predictors], train["winner"])
    predictions = rf.predict(test[predictors])
...
most_confidense_accuracy = counts.get(1, 0) / (counts.get(1,0) + counts.get(0, 0))
print("\nAccuracy of predicting the winner and loser on a game ", round(most_confidense_accuracy, 4))